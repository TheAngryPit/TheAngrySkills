...
function reviewCell(row: T.PrSnapshot): string {
  ...
  if (row.reviewAutomationRunning)
    return open ? `🤖 running, ${open} open` : "🤖 running";
  if (open) return `📝 ${open} open`;
  switch (row.facts.reviewDecision) {
    case "APPROVED":
      return "✅ approved";
    ...
    case null:
      return "— no decision";
    default: {
      const exhaustive: never = row.facts.reviewDecision;
      return exhaustive;
    }
  }
}
        ...
        blocker.reason === "closed-without-merge"
          ...
          : blocker.reason === "draft-pr"
            ...
            : blocker.reason === "review-required"
              ? "obtain the required review before waiting for the merge queue"
