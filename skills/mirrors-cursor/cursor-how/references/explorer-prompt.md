...
Start by finding the relevant code. Use the available native file search and read tools (for example `rg --files`, `rg` and file reads) to locate directories, symbols and implementations. Don't guess from names. Read the code.
