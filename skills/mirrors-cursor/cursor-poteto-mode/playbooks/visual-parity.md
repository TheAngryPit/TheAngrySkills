...
4. Verify each component against its baseline via image diff on the matching surface via the control skill. A nonzero diff is a fail. Investigate the pixel delta. an explicitly requested Codex heartbeat per component until the diff is zero.
