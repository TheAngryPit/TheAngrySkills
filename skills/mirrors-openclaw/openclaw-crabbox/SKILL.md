...
crabbox_src="$openclaw-crabbox_root"
if [ -d "$openclaw-crabbox_root/.git" ]; then
  :
elif [ -d "$openclaw-crabbox_root/src/.git" ]; then
  crabbox_src="$openclaw-crabbox_root/src"
elif [ ! -e "$openclaw-crabbox_root" ]; then
  git clone https://github.com/openclaw/crabbox.git "$openclaw-crabbox_root"
else
  echo "Existing $openclaw-crabbox_root is not a Crabbox checkout; move it aside first." >&2
  exit 1
fi
if [ -n "$(git -C "$openclaw-crabbox_src" status --short)" ]; then
  git -C "$openclaw-crabbox_src" status --short
  echo "Dirty $openclaw-crabbox_src checkout; resolve before updating Crabbox." >&2
  exit 1
fi
git -C "$openclaw-crabbox_src" pull --ff-only
mkdir -p "$openclaw-crabbox_root/bin"
crabbox_bin="$(cd "$openclaw-crabbox_root" && pwd)/bin/crabbox"
go version
(cd "$openclaw-crabbox_src" && go build -o "$openclaw-crabbox_bin" ./cmd/crabbox)
"$openclaw-crabbox_bin" --version
...
node scripts/crabbox-wrapper.mjs run \
  ...
  --timing-json \
  -- \
  CI=1 NODE_OPTIONS=--max-old-space-size=4096 OPENCLAW_TEST_PROJECTS_PARALLEL=6 OPENCLAW_VITEST_MAX_WORKERS=1 pnpm check:changed
```
