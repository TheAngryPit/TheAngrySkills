---
name: cursor-principle-build-the-lever
...
Build the smallest rerunnable tool or check before repeating work. Required native capabilities: none; this is instruction-only. No external product dependency is required by the adapted path. No tool is required; a nondeterministic lever is unproven. This candidate is promoted for the bounded path described here; unrelated runtime parity remains unproven. This is a guide-only adaptation.
# Build the Lever
...
**Balance:** The bar is triviality, not repetition. A one-off still earns a lever when the lever is what makes the work checkable. Per the [Laziness Protocol](../cursor-principle-laziness-protocol/SKILL.md), build the smallest script that does or proves the job, never a framework.

Distinct from [Encode Lessons in Structure](../cursor-principle-encode-lessons-in-structure/SKILL.md), which makes a recurring instruction a durable guardrail. This is throughput and reviewability on the work in front of you. For scripting the verification itself, see [Prove It Works](../cursor-principle-prove-it-works/SKILL.md).
