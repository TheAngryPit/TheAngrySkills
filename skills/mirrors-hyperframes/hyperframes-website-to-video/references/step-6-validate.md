...
[ ] w2h-verify report                     → run `node <repo-root>/skills/hyperframes-website-to-video/scripts/w2h-verify.mjs <project-dir>`; paste the FULL output (every row, every percent) verbatim into your final user-facing summary — see "w2h-verify — the source of truth" below
...
```bash
node <repo-root>/skills/hyperframes-website-to-video/scripts/w2h-verify.mjs <project-dir>
```

(Locate the repo root from a project subdirectory: `find "$HOME" -path '*/skills/hyperframes-website-to-video/scripts/w2h-verify.mjs' -maxdepth 10 2>/dev/null | head -1`.)
