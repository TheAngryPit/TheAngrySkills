...
Use native task history and verified rollout paths, not `~/.cursor/projects`. Preserve the scope window, distinct own-history/shared-record sweep, citations, live-state readback and capsule format. Apply the standing model router to optional read-only delegation.
...
Your context lives in two records. Your own chat history holds what you did and decided. The shared record holds everything that happened around the same code under other names: the symptoms users keep reporting, the fixes that shipped and got reverted, the errors still firing in prod. That second record is what `cursor-why` searches, across source control, the issue tracker, chat and issue channels, long-form docs, and error tracking. A feature with a long bug tail keeps most of its story there, so don't reconstruct it from your transcripts alone.

In Codex, discover accessible prior tasks with the native thread list/read tools. Local rollout JSONL may provide full tool history when a verified rollout path is available; its filesystem layout varies by host. Do not infer a Cursor transcript path or read a different project without the user’s scope.

1. Classify, then route. One specific prior chat to resume uses the [session-pickup playbook](../cursor-poteto-mode/playbooks/session-pickup.md) from `cursor-poteto-mode`, not this skill. Turning habits into a durable skill is `cursor-automate-me`. A human-readable summary of your work is a different task. Recall loads working context across recent chats before you act. If the user already gave you a full state capsule (paths, branch, the change), use it and skip the mining.
...
3. Fan out across your chat history. For a large in-scope corpus, delegate independent slices to read-only Codex subagents under the standing `model-capability-router` policy. Tell each investigator to order accessible tasks by verified update time, search the topic before opening a transcript, read only relevant regions, and skip the current task plus obvious eval/test noise. Do not assume a filename UUID encodes recency. Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
4. Sweep the shared record whenever the topic names a feature, file, subsystem, area, or bug. This is the default, not a judgment call, and "my work on X" does not exempt it. Hand it to `cursor-why` source investigators, but steer their question from "why was this built this way" to "what's the current state, what's been tried and didn't hold, and what are users still reporting". Reuse its per-source playbooks, run the investigators in parallel with the chat-history mining, and inherit its posture: one investigator per source, null results are findings, skip an unavailable MCP and say so. Fold what comes back into the brief. Skip this step only for pure activity recall with no named target ("what did I do this week"), where your own history and live state are the entire answer.
...
An adjacent feature or ticket stays out unless it blocks this one. When the capsule and thread lines outgrow a screen, cut detail before you cut threads. Write the brief through `cursor-unslop`, cite chat findings by UUID and shared-record findings by their source (PR #, ticket ID, chat permalink, error-tracker issue), and sanitize private context before any public output.
