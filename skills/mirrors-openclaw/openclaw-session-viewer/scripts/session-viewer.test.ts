...
test("CLI preserves full sessions over 8 MiB by default in both export modes", async (t) => {
  ...
  for (const mode of [[], ["--raw"]]) {
    const { stdout } = await execFileAsync(process.execPath, [
      "skills/openclaw-session-viewer/scripts/session-viewer.ts", input, "--out", output, ...mode,
    ]);
...
test("CLI rejects invalid explicit byte limits without writing an export", async (t) => {
  ...
  for (const value of ["0", "-1", "1.5", "NaN", "Infinity", "9007199254740992"]) {
    await assert.rejects(execFileAsync(process.execPath, [
      "skills/openclaw-session-viewer/scripts/session-viewer.ts", "unused.jsonl", "--out", output,
...
test("CLI preserves exact-limit raw input without truncation", async (t) => {
  ...
  await fs.writeFile(input, text);
  await execFileAsync(process.execPath, [
    "skills/openclaw-session-viewer/scripts/session-viewer.ts", input, "--out", output,
...
test("copied viewer renders inside a CommonJS project without dependencies", async (t) => {
  ...
  await fs.cp(path.resolve("skills/openclaw-session-viewer"), installed, { recursive: true });
