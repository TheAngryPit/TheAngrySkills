...
1. **Scaffold** — `HYPERFRAMES_SKIP_SKILLS=1 npx hyperframes init my-video` (or `capture` from a URL). the mirror sets `HYPERFRAMES_SKIP_SKILLS=1` so `init` scaffolds without changing the installed skill source. Keep the environment guard in place for mirrored installs; omitting it lets the product CLI manage skills from the upstream source.
