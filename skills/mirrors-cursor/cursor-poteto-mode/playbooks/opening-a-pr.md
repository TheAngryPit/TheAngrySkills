...
**PRs.** Run the concrete mirror skill `cursor-deslop` over the diff before commit when installed; otherwise record that cleanup capability is unavailable. Run `cursor-no-comments` before review when installed and applicable. Write every PR title, PR description, and commit body with `cursor-technical-writing`, then apply `cursor-unslop`; record the exact gap if a held or unavailable skill cannot be invoked. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.
...
A bounded delegate that opens a PR uses `cursor-interrogate`, `cursor-deslop`, and `cursor-no-comments` only when each concrete skill is installed and applicable; otherwise it records the gap. It returns the URL and does not babysit. Return to the parent.
