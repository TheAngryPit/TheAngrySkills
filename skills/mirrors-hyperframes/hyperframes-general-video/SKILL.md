...
> **figma source**: If any input is a figma.com URL, run `/hyperframes-figma` first — asset export, brand tokens, and components/storyboard reconstruction if needed — then build from its output. Don't drive Figma via raw MCP tools directly: that skips SVG sanitization, `.media/manifest.jsonl` provenance, and brand-token `var()` binding, so a later brand change can't propagate without a full re-import.

# general-video — general video workflow

> **Confirm the route before you build.** This is the **fallback** for custom composition authoring. If the input clearly fits a specialized workflow, prefer it: marketed product → `/hyperframes-product-launch-video`; general site → `/hyperframes-website-to-video`; topic explainer → `/hyperframes-faceless-explainer`; GitHub PR → `/hyperframes-pr-to-video`; existing footage → `/hyperframes-embedded-captions` · `/hyperframes-talking-head-recut`; short unnarrated motion graphic → `/hyperframes-motion-graphics`; Remotion port → `/remotion-to-hyperframes`. **Out of scope**: live / at-render-time data, NLE-style editing of a finished video, or producing footage HyperFrames can't capture. Unsure? **Read `/hyperframes` first.**
