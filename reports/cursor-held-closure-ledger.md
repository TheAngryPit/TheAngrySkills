...
| `cursor-orchestrate` | Only explicit `/orchestrate <goal>` may decompose work into parallel Cursor cloud workers/subplanners/verifiers and collect structured handoffs through `scripts/cli.ts`. | External-product adapter produces plan-only when delegation/credentials are absent; bundled runtime remains reference-only. | Command boundary, dependency/egress, Slack, worker hierarchy, isolation, cancellation, recovery and cloud artifact/PR behavior. | Bundled scripts expose `bash -c`, `CURSOR_API_KEY`, Slack access, dependencies and writes; Official Cursor cloud API contract exists, but no authenticated run/retry/egress proof is available. | Keep plan-only; separately review the scripts and run a disposable no-egress orchestration simulation. |
