...
Applies to the narrated, story-driven creation workflows — `/hyperframes-product-launch-video`, `/hyperframes-pr-to-video`, `/hyperframes-faceless-explainer`, `/hyperframes-website-to-video`, and `/hyperframes-general-video` when the piece tells a story. It does **not** apply to `/hyperframes-music-to-video` (the track drives the arc), `/hyperframes-motion-graphics` (no narration — motion is the message), `/hyperframes-embedded-captions` and `/hyperframes-talking-head-recut` (the footage's story is already fixed), or `/hyperframes-slideshow` (the presenter owns the story). Do not force these rules onto an exempt workflow.
