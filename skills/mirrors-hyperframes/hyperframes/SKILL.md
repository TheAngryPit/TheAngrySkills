...
| **Media** — resolve/generate BGM, SFX, image, icon, brand logo, voice, color grade, LUT; TTS voiceover, transcription, background removal, captions; cross-project reuse | `/hyperframes-media-use`             |
...
| **Import Figma content** — assets, tokens, components, storyboards→reconstructed motion (REST/CLI); Motion (MCP), shaders (MCP source / native export)                   | `/hyperframes-figma`                 |
...
| Workflow                   | Use it for                                                                                                                                                                                                                                          |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/hyperframes-product-launch-video`    | **Selling a product** (SaaS, app, company / product site) — from a URL, brief, or script → a **promo**. The default for any commercial URL, even if the site is only named.                                                                         |
| `/hyperframes-website-to-video`        | **Showing a site itself** — a tour / showcase built from the site's own screenshots. For non-commercial sites (portfolio, blog, docs, personal, event), or when the user wants a tour, not a promo.                                                 |
| `/hyperframes-faceless-explainer`      | **Explaining a topic / concept** from text — no product, no URL; every visual is LLM-invented                                                                                                                                                       |
| `/hyperframes-pr-to-video`             | A **GitHub PR / code change** → changelog / feature-reveal / fix / refactor explainer                                                                                                                                                               |
| `/hyperframes-embedded-captions`       | Adding **captions / subtitles** to an existing talking-head video (footage untouched)                                                                                                                                                               |
| `/hyperframes-talking-head-recut`      | Packaging an existing talking-head video with **designed graphic overlays** — lower-thirds, data callouts, kinetic titles, pull-quotes                                                                                                              |
| `/hyperframes-motion-graphics`         | A short (~under 10s), **unnarrated** piece where the **motion _is_ the message** — kinetic type, a stat / chart hit, a logo sting, an animated map, an animated tweet / headline, a **standalone** lower-third / overlay (MP4 or transparent alpha) |
| `/hyperframes-music-to-video`          | A **music track** → a **beat-synced** video — lyric video, slideshow, or kinetic promo; the music drives pacing (optional user images / videos cut onto the beat grid)                                                                              |
| `/hyperframes-slideshow`               | A **presentation / pitch deck / interactive deck** — discrete slides, fragments, branching, hotspots; output is a navigable **deck**, not a rendered video                                                                                          |
| `/hyperframes-general-video`           | **Anything else** — longer or multi-scene pieces, a static loop / poster, a custom composition                                                                                                                                                      |
...
- **Motion-first & unnarrated** (under ~10s, the motion _is_ the message) → `/hyperframes-motion-graphics`, regardless of input.
- **A URL or script** — ask one thing: _is the site selling a product?_ **Yes** (SaaS / app / product / company site) → `/hyperframes-product-launch-video` — a promo, and the default for any commercial URL even if the site is only named. **No**, or the user just wants the site shown as-is (portfolio / blog / docs / personal / event) → `/hyperframes-website-to-video` — a tour. A GitHub PR link → `/hyperframes-pr-to-video`; a concept with no product or site → `/hyperframes-faceless-explainer`.
- **Existing footage** — plain spoken-word subtitles → `/hyperframes-embedded-captions`; designed overlay cards → `/hyperframes-talking-head-recut`. Neither edits the footage itself (re-timing / recolor / reframe / reorder / audio is NLE editing — out of scope).
- **A music track is the input** (an audio file, or a video to pull audio from) with **no narration** → `/hyperframes-music-to-video` — the music's beats/energy drive the pacing. (Narrated pieces stay with the input-matched workflow above; `/hyperframes-motion-graphics` is for short unnarrated motion that isn't music-driven.)
- **A presentation / pitch deck / interactive deck** (discrete slides, navigation, presenter mode) → `/hyperframes-slideshow` — output is a navigable deck, not a rendered video. An explicit "hyperframes-slideshow" request proceeds directly; an adjacent trigger ("deck / slides / presentation / convert this page") makes `/hyperframes-slideshow` confirm it's a slideshow before authoring, and switch to the appropriate non-slideshow workflow if not.
- **Length is a guide, not a gate** — intent picks the workflow; go to `/hyperframes-general-video` only when the piece is clearly longer than ~3 min, or is a static / loop / custom format.
...
```bash
npx skills add https://github.com/TheAngryPit/TheAngrySkills.git --skill <mirrored-workflow-name> --skill hyperframes --skill hyperframes-animation --skill hyperframes-cli --skill hyperframes-core --skill hyperframes-creative --skill hyperframes-keyframes --skill hyperframes-registry --skill hyperframes-media-use --yes
```

Bare name, no leading `/` — e.g. `npx skills add https://github.com/TheAngryPit/TheAngrySkills.git --skill hyperframes-pr-to-video --skill hyperframes --skill hyperframes-animation --skill hyperframes-cli --skill hyperframes-core --skill hyperframes-creative --skill hyperframes-keyframes --skill hyperframes-registry --skill hyperframes-media-use --yes`. Naming a skill guarantees it **plus the core domain skills** every workflow depends on are installed and current: a fast no-op when everything already is, a targeted install of just the missing/stale skills when not — never the full set. Then read the workflow's skill and continue. The same command works for an on-demand domain skill from the capability map (e.g. `npx skills add https://github.com/TheAngryPit/TheAngrySkills.git --skill hyperframes-figma --skill hyperframes --skill hyperframes-animation --skill hyperframes-cli --skill hyperframes-core --skill hyperframes-creative --skill hyperframes-keyframes --skill hyperframes-registry --skill hyperframes-media-use --yes`).

If the command fails, surface its error to the user instead of improvising the workflow from memory. Do not fall back to the upstream skill source, because that would install duplicate unprefixed skills. Retry the mirrored command or run `npx skills update`.
...
- **Install the full mirrored pack explicitly:** `npx skills add https://github.com/TheAngryPit/TheAngrySkills.git --skill hyperframes-embedded-captions --skill hyperframes-faceless-explainer --skill hyperframes-figma --skill hyperframes-general-video --skill hyperframes --skill hyperframes-animation --skill hyperframes-cli --skill hyperframes-core --skill hyperframes-creative --skill hyperframes-keyframes --skill hyperframes-registry --skill hyperframes-media-use --skill hyperframes-motion-graphics --skill hyperframes-music-to-video --skill hyperframes-pr-to-video --skill hyperframes-product-launch-video --skill remotion-to-hyperframes --skill hyperframes-slideshow --skill hyperframes-talking-head-recut --skill hyperframes-website-to-video --yes`.
...
- **Output:** a product launch / SaaS **promo** as a HyperFrames composition → MP4 (sweet spot 30–90s) — the product's value is the subject, not a walkthrough of the site. For a plain tour of the site, use `/hyperframes-website-to-video`.
...
- **Input:** A website / URL whose goal is to show **the site itself**, not to sell a product. Best for non-commercial sites (portfolio, blog, docs, personal, event), or when the user explicitly wants a tour of a site as-is. Captured with headless Chrome for real screenshots + brand assets. If the site is selling something and the user wants a promo, use `/hyperframes-product-launch-video`.
...
- **Input:** Arbitrary text — a topic, article, or notes — being **explained**, with no product being marketed and no site to capture. (Forked from `/hyperframes-product-launch-video`; no headless Chrome.)
