...
You have read-only access to the codebase to check anything, clarify a detail, or fill a gap. Use the native Codex file search and read tools available in this task as needed. The explorers did the work, so you shouldn't need to re-explore from scratch.
