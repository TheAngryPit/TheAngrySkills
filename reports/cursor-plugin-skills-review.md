...
candidates. The 11 declared findings remain 5 `blocked_malicious`, 5
...
|---|---|---|---|
| `add-dictation` | `network-exfiltration`, `SKILL.md:156`, critical | Active `curl` smoke instruction sends an audio file to xAI STT with `XAI_API_KEY`. | Hold. No live request; require an approved server relay and provider/network review. |
| `add-read-aloud` | `network-exfiltration`, `SKILL.md:172`, critical | Active `curl` smoke instruction sends reply text to xAI TTS with `XAI_API_KEY` and writes returned audio. | Hold. No live request; require an approved server relay and provider/network review. |
...
with no errors, and the security scanner returned `safe_to_install` for 53/53.
Ten focused tests cover reproducibility, local-edit preservation, source-drift
