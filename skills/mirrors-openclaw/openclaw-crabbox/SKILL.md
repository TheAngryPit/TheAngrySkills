...
```sh
command -v crabbox
../openclaw-crabbox/bin/crabbox --version
...
```sh
if [ -x ../openclaw-crabbox/bin/crabbox ]; then
  export CRABBOX=../openclaw-crabbox/bin/crabbox
else
...
```sh
if [ -n "$(git -C ../openclaw-crabbox status --short)" ]; then
  git -C ../openclaw-crabbox status --short
  exit 1
fi
git -C ../openclaw-crabbox pull --ff-only
mkdir -p ../openclaw-crabbox/bin
(cd ../openclaw-crabbox && go build -o bin/crabbox ./cmd/crabbox)
../openclaw-crabbox/bin/crabbox --version
```
