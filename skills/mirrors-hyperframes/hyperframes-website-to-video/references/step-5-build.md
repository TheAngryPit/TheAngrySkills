...
```bash
cp -r skills/hyperframes-website-to-video/assets/sfx/ <project-dir>/sfx/
# If skill is installed elsewhere:
find . -path "*/hyperframes-website-to-video/assets/sfx" -exec cp -r {} <project-dir>/sfx/ \;
```
...
the skill lives outside it — run this to find it:
  ...
  find "$HOME" -path '*/hyperframes-website-to-video/references/beat-builder-guide.md' -maxdepth 10 2>/dev/null | head -1
