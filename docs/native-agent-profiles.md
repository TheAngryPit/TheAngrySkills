...
| `cursor-interrogate`, `cursor-reflect` | Interrogate passes one identical rubric to each configured reviewer; Reflect uses judgment/tooling/divergent templates and a separate synthesizer. | Native delegates keep independent scopes and templates; actual model identity must come from host metadata. No generic-to-poteto substitution. | [Interrogate proof](../reports/pstack-interrogate-native-20260914.md) covers two same-scope reviewers, distinct requested profiles, separate outputs, synthesis and no auto-apply. Effective backend model metadata and Reflect's three-lens execution remain open. |
