...
def run_native_migration(
    ...
    *,
    ...
    if not isinstance(execute, bool) or not isinstance(authorization_granted, bool):
        raise AdapterError("execute and authorization_granted must be boolean")
    try:
    ...
    if plan["authorization_required"] and not authorization_granted:
        return _common_result(
