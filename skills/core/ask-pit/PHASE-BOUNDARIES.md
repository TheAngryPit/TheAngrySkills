...
The **phase boundary** is the gap between two phases, and it is the only place this decision belongs. Prefer natural boundaries when practical. If the harness compacts mid-phase or a handoff is needed, preserve the current objective, decisions and unfinished work.
...
Use these options at a boundary, taking account of the current task, available context and native harness capabilities.

**1. Can you continue in this session?** Two things make the answer yes: the next phase needs this phase as a **primary source**, or the current context remains useful and has enough capacity for the next phase. Grilling → implementation is the standard yes: the implementation wants the reasoning verbatim, not a summary of it. Continue preserves the current context; weigh its value against context capacity and processing cost.
...
These are common portability cases. Also respect the user's established handoff workflow when continuity requires it.
...
The questions are not objective: each has taste in it, and the same boundary can go two ways on two days. The value is in preserving the information needed for the next phase, using the controls the current harness actually supports.
