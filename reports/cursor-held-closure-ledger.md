...
| `cursor-create-verification-skill` | Generate `.cursor/skills/verify-<app>/` that launches the real app, drives each feature as a user, captures evidence and cleanup. | Native-write adapter maps to project-local `.agents/skills/verify-<app>/`; disposable notes CLI observes syntax/version/health Doctor, all mapped CLI actions, declared cleanup and surviving evidence. | Live target/harness, native activation and permission proof. | No live target application or native verification runner is supplied. | Run against an explicitly authorized disposable app, then verify launch/doctor/drive/evidence/cleanup and second pass. |
...
| `cursor-maintain-verification-skill` | For every feature, read source in parallel, drive every feature live, reconcile feature map and make at most one PR of proven corrections. | Project-local maintenance adapter and CLI fixture capture Doctor, caller-provided source-wave input, controlled drift, two feature re-drives, explicit cleanup and clean second invocation. | Live app, native source-wave delegation, native runner and changed-outcome PR boundary. | No live target/native runner path is supplied; the source-wave input is not a native reader result. | Run a disposable feature-map maintenance pass, then prove live feature coverage before any PR. |
...
`cursor-how`, `cursor-why`, `cursor-show-me-your-work`, `cursor-thermos`, and `cursor-swarm`
...
2. **Delegated review and style:** `cursor-advisor`, `cursor-architect`,
   `cursor-arena`, `cursor-interrogate`, `cursor-reflect`,
...
All 29 rows remain absent from the published catalog and unavailable for a
...
The list contains exactly the 29 active `publish:false` names and excludes the
...
- Manifest count: `jq` reports 91 physical skills, 32 `publish:false`, and 29
